...
fn get_cardano_nft_addresses() -> HashSet<String> {
    let mut path = env::current_exe().expect("Failed to get the current executable path");
    path.pop(); // Remove the executable name
    path.pop(); // Go up one directory (out of target/debug or target/release)
    path.pop(); // Go up one more directory (out of target)
    path.push("node");
    path.push("src");
    path.push("node_operator_cardano_nfts.json");

    println!("INFO: Attempting to open Cardano NFT addresses file at: {:?}", path);
